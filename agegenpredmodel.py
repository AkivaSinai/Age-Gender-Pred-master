...
class AgeGenPredModel(torch.nn.Module, ):
  ...
  def get_resnet_convs_out(self, x, attention_coordinates= None):  #new argument todo- add default attention
    """
    ...
    # #new
    #attention_coordinates_per_channel=  tile(attention_coordinates[:,None, :, :],1,3)

    attention_mask = nn.MaxPool2d(kernel_size=7, stride=2, padding=3)(attention_coordinates)
    x = self.resNet.conv1(x)    # out = [N, 64, 112, 112]
    if(attention_mode>=1):
      attention_mask = nn.MaxPool2d(kernel_size=7, stride=2, padding=3)(attention_coordinates)
      attention_mask= tile(attention_mask[:,None, :, :],1,64, self.use_gpu)
    if (attention_mode == 1):
      x= x*attention_mask.float()
    x = self.resNet.bn1(x)
    x = self.resNet.relu(x)
    if (attention_mode >= 2):
      attention_mask= self.resNet.maxpool(attention_mask)
    x = self.resNet.maxpool(x)
    if (attention_mode == 2):# out = [N, 64, 56, 56]
      x = x * attention_mask.float()
  ...
  def forward(self, x, attention_area):
    last1 = self.get_resnet_convs_out(x, attention_area)
