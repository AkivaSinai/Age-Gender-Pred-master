...
# distribution of val dataset: FG-NET
...
class FaceDataset(Dataset):
  ...
  def __init__(self, datapath, transformer):
    ...
    # new
    self.setEyeMap(original_mode,5)
  ...
  def setEyeMap(self,original_mode, margin=0):
      self.eye_map = {}
      blank_map = np.ones((256, 256), np.uint8)*0.5
      for idx in range(len(self.pics)):
          if (original_mode==False):
              img_name = self.datapath + self.pics[idx]
              img = io.imread(img_name)
              eye_area = np.array(self.getEyeArea(img))
              # if(eye_area.size==0):
              #     eye_area=np.array([[-1,-1,-1,-1]])
              # while(len(eye_area)<ATTENTION_AREAS):
              #      eye_area=np.append(eye_area,[[-1,-1,-1,-1]], axis=0 )
              # while(len(eye_area)>ATTENTION_AREAS):
              #      eye_area=np.delete(eye_area,2,0 )
              if(len(eye_area)>=0):
                  for (ex, ey, ew, eh) in eye_area:
                      begin_x = max(ex - margin, 0)
                      end_x = min(ex + ew + margin, 255)
                      begin_y = max(ey - margin, 0)
                      end_y = min(ey + eh + margin, 255)
                      blank_map[begin_y:end_y, begin_x:end_x] = 1
          self.eye_map[self.pics[idx]]= blank_map



  def getEyeArea(self, img):
  ...
  def __getitem__(self, idx):
    ...
    #new
    eye_area= self.eye_map[self.pics[idx]]
    ...
    attention= np.array(self.eye_map[self.pics[idx]], dtype=np.float)
    # preprocess labels
    ...
    return image, attention, gender, age_rgs_label, age_cls_label
